 ...
 namespace CoLib
 {
     ...
     public static class MaterialExtensions
     {
         ...
         private static void CheckPropertyExists(Material material, string property)
         {
             ...
             if (!material.HasProperty(property))
             {
                 throw new InvalidOperationException(
                     string.Format("Material doesn't have property named {0}", property));
             }
         }
         ...
         private static void CheckPropertyExists(Material material, int property)
         {
             if (!material.HasProperty(property))
             {
                 throw new InvalidOperationException(string.Format("Material doesn't have property with ID {0}",
                     property));
             }
         }
